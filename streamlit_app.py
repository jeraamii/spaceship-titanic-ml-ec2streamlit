...
def invoke_endpoint(features: list) -> dict:
    # MOCK
    import random
    prob_transported = random.uniform(0.3, 0.9)
    return {
        "probabilities": [[1 - prob_transported, prob_transported]],
        "predictions": [1 if prob_transported > 0.5 else 0],
        "labels": ["Transported" if prob_transported > 0.5 else "Not Transported"],
    }


st.set_page_config(page_title="Spaceship Titanic Classifier", page_icon="🚀")
st.title("Spaceship Titanic – Transported Predictor, by Michael Yeremia")
st.markdown(
